...
export const getAllTasks = async ( req, res, next) => {
    try {
        // empty braces to return all tasks
...
export const updateTask = async ( req, res, next ) => {
    try{
        ...
        // user_id coming from the task   //user_id coming from payload 
        if(task.user.toString() !== req.user.id){
            return next(createError({ status: 401, message: "it's not allowed to edit others' tasks"}))
        }
